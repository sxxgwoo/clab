...
class RlDataGenerator:
    ...
    def __init__(self, file_folder_path="./data/real/filtered"):
    ...
    def batch_generate_rl_data(self):
        ...
        csv_path= self.file_folder_path + '/train_set_offline.csv'
        print(csv_path)
    ...
    def _generate_rl_data(self, df):
        ...
        for id_value, group in grouped:
            ...
            for index, row in group.iterrows():
                ...
                remain_time_from_TimeMachine = row['req_leaving_time_timemachine']- row['cur_time']
                
            
                eu_distance = np.sqrt((row['start_x'] - row['target_x'])**2 + 
                                    (row['start_y'] - row['target_y'])**2)
                
                var = row['oracle']- row['cur_time']
                ...
                training_data_rows.append({
                    ...
                    'group': row['group'],
                    'round': row['round'],
                    'req_leaving_time_timemachine':row['req_leaving_time_timemachine'],
                    'oracle': row['oracle'],
...
def generate_rl_data():
    file_folder_path = "/home/sxxgwoo/clab/data/real/filtered"
